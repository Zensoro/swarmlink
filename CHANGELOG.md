...
### 完成
- **Docker 构建验证**（此前文件就绪但网络阻断未验证）：
  - Docker Desktop 配置手动代理 (127.0.0.1:10815) 打通 Docker Hub
  - `docker compose up` 双容器实测：sky 发送 427 包（重传 7, ARQ 生效），
    gnd 完成 30/30 帧, 控制消息 5/5, 解密 351 成功 0 失败
- **README 应用场景**：新增"可能适用的应用场景"节
  （无人机图传 / 应急救援 / 安防监控 / 赛事直播 / 传感器遥测 / 边缘部署）
