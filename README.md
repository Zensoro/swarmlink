...
SwarmLink 定位: **一对多、弱网韧、低延迟、可扩展** 的传输层。
以下场景是它的自然适用面:

### 无人机 / 机器人图传
- 一架无人机视频流 → 多副地面眼镜/多台地面站 (一对多核心场景)
- 远距离弱网 (山区/景区) 靠 FEC+ARQ 兜底, SFU 让不同端看不同码率
- 控制/遥测走 ReliableChannel 可靠通道, 与视频流互不干扰

### 应急救援 / 现场指挥
- 单兵摄像头 → 指挥中心多屏同时观看
- 消防/电力巡检: 现场画面回传, 断连恢复靠 stale-while-revalidate 旧帧缓冲
- 多中继组网 (三级拓扑) 覆盖复杂地形

### 安防监控 / 移动巡检
- 移动巡逻车/船载多路视频 → 后端多路并存
- 4G/5G 蜂窝弱网场景, GE 突发丢包模型贴近真实信道

### 赛事 / 演出多视角直播
- 多机位 → SFU 按订阅分发不同清晰度 (观众端弱网降档)
- 低延迟优先, 控制流 (切机位指令) 可靠必达

### 传感器集群 / 遥测回传
- 多设备遥测流 → 汇聚中心 (多流复用 + 可靠通道)
- 海量小消息: ReliableChannel 单包 ARQ 语义, 顺序交付

### 嵌入式 / 边缘部署
- 纯 Python 参考实现 + Rust 核心 (RS/RLNC 88-145x) 覆盖实时吞吐
- Docker 一键部署, Web 仪表盘监控链路健康
